...
//----------------------------------------------------------------------------------
//
// CRUNKCBOXB Background system box
//
//----------------------------------------------------------------------------------
#import "CBitmap.h"
#import "CRunKcBoxB.h"
...
-(BOOL)createRunObject:(CFile*)file withCOB:(CCreateObjectInfo*)cob andVersion:(int)version
{
	...
	if ((rData->dwFlags & FLAG_CONTAINED) != 0)
	{
		...
		if (rContNum != -1)
		{
			CRunKcBoxB* rdPtrCont = (CRunKcBoxB*)[pData->pContainers get:rContNum];
			rContDx = (short) [ho getX] - [rdPtrCont->ho getX];
			rContDy = (short) [ho getY] - [rdPtrCont->ho getY];
		}
	}
	rData1 = [[KcBoxBCData1 alloc] init];
...
-(void)destroyRunObject:(BOOL)bFast
{
	...
	// Get FrameData
	KcBoxBCFrameData* pData = (KcBoxBCFrameData*)[rhPtr getStorage:ho->hoIdentifier];
...
{
	CRect rc = CRectCreateAtPosition([ho getX], [ho getY], [ho getWidth], [ho getHeight]);
	CFontInfo* hFn=wFont;
...
-(void)DisplayObject:(CRenderer*)renderer withParam1:(CRunApp*)idApp andParam2:(CRect)rc andParam3:(KcBoxBCData*)pc andParam4:(NSString*)text andParam5:(CFontInfo*)hFnt andParam6:(KcBoxBCData1*)pdata1
{
	int x = (int)rc.left;
	int y = (int)rc.top;
	int w = (int)rc.width();
	...
	if ((pc->wImage != nil) && ((pc->dwFlags & FLAG_HIDEIMAGE) == 0))
	{
		...
		if (wc > 0 && hc > 0)
		{
			...
			{
				renderer->renderImage(rData->wImage,
									  x+(w-rData->wImage->width)/2,
									  y+(h-rData->wImage->height)/2,
									  rData->wImage->width,
									  rData->wImage->height,
									  0, 0);
			}	
...
// CLASSES ANNEXES ////////////////////////////////////////////////////////////////////////////
@implementation KcBoxBCData
@end

@implementation KcBoxBCData1
@end

@implementation KcBoxBCFrameData
...
-(int)AddObjAddr:(int)t withParam1:(CRunKcBoxB*)reObject
{
	if (t == TYPE_OBJECT)
	{
		// 1st allocation
...
-(void)RemoveObjAddr:(int)t withParam1:(CRunKcBoxB*)reObject
{
	if (t == TYPE_OBJECT)
	{
		if (pObjects != nil)
...
-(int)GetContainer:(CRunKcBoxB*)re
{
	...
	if (pContainers != nil)
	{
		for (int i=0; i < [pContainers size]; i++)
		{
			...
			{
				CRunKcBoxB* reThisOne = (CRunKcBoxB*)[pContainers get:i];
...
-(int)GetObjectFromList:(int)x withParam1:(int)y
{
	...
	if (pObjects != nil)
	{
		for (int i = [pObjects size] - 1; i >= 0; i--)
		{
			if ([pObjects get:i] != nil)
			{
				CRunKcBoxB* reThisOne = (CRunKcBoxB*)[pObjects get:i];
			...
			if ([pObjects get:i] != nil)
			{
				...
				if ((reThisOne->rData->dwFlags & FLAG_CONTAINED) != 0)
				{
					...
					if (reThisOne->rContNum == -1 )
					{
						...
						if (reThisOne->rContNum != -1 )
						{
							CRunKcBoxB* rdPtrCont = (CRunKcBoxB*)[pContainers get:reThisOne->rContNum];
